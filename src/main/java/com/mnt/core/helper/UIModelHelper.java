...
public  abstract class UIModelHelper<M extends ProxyUI> implements UIModel {
	...
	protected void buildFields(){
		for(Field f :M.getFields()){
				...
				if(f.isAnnotationPresent(OneToOne.class))
				{
					try 
					{
						if(value.getO() != null){
							...
							value.setId(clazz.getDeclaredField("id").get(value.o));
						}
					}catch (Exception e)
					{
						e.printStackTrace();
			...
			if(f.getType().getSuperclass() != null  && SearchUIHelper.class.getCanonicalName().equals(f.getType().getSuperclass().getCanonicalName())){
				try {
					if(valueObject==null){
						value.setO(f.getType().newInstance());
					}
				} catch (Exception e) {
					e.printStackTrace();
				} 
	...
	public Class<? extends ProxyUI> getM() {
		return M;
	}

	public void setM(Class<? extends ProxyUI> m) {
		M = m;
	}

	public ProxyUI getValueObject() {
		return valueObject;
	}

	public void setValueObject(ProxyUI valueObject) {
		this.valueObject = valueObject;
	}
	...
	public void setUiButtons(List<UIButton> uiButtons) {
		this.uiButtons = uiButtons;
	}

	public List<Set<FieldType>> getFieldLayoutList() {
		return fieldLayoutList;
	}

	public void setFieldLayoutList(List<Set<FieldType>> fieldLayoutList) {
		this.fieldLayoutList = fieldLayoutList;
	}

	public void setFieldLayout(Map<Integer, Set<FieldType>> fieldLayout) {
