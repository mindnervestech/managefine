/**
 * @preserve Basic Primitives orgDiagram v2.0.17
...
primitives.orgdiagram.BaseController = function () {
	...
	this._orgItemRoot = null; // primitives.orgdiagram.OrgItem
...
primitives.orgdiagram.BaseController.prototype._setItemsIntervals = function () {
	if( typeof this._itemsInterval == 'undefined') {
		this._itemsInterval = [];
	}
		...
		for (index = 0, len = this._treeLevels.length; index < len; index += 1) {
			...
			if (treeLevel.activeTreeItems.length > 0) {
			    if (y > treeLevel.topConnectorShift && y <= treeLevel.connectorShift) {
			        result = self._treeItems[primitives.common.binarySearch(treeLevel.activeTreeItems, function (itemId) {
			            var treeItem = self._treeItems[itemId];
			            switch (treeItem.actualVisibility) {
			                case 1/*primitives.common.Visibility.Normal*/:
...
primitives.orgdiagram.BaseController.prototype._stretchToAvoidPageMargins = function (diagramSize, panelSize) {
    ...
    sortedItems = primitives.common.mergeSort(treeLevelsItems, function (item) { return self._treeItems[item].offset; });

    globalOffset = 0;
    pageRightMargin = pageSize.width;
    for (index = 0, len = sortedItems.length; index < len; index += 1) {
        item = self._treeItems[sortedItems[index]];
...
primitives.orgdiagram.BaseController.prototype._createVisualTree = function () {
    ...
    if (this._orgItemRoot != null) {

        rootTreeItem = this._getNewTreeItem({
        ...
        this._createVisualTreeItem(rootTreeItem);
...
primitives.orgdiagram.BaseController.prototype._getNewTreeItem = function (options) {
    ...
    if (orgItem != null) {
        ...
        result.actualHasButtons = (this.options.buttons.length > 0) && this._getSelectionVisibility(this.options.cursorItem == orgItem.id, orgItem.hasButtons, this.options.hasButtons);
...
primitives.orgdiagram.BaseController.prototype._readVisualTree = function (treeItem, level) {
	...
	if (treeItem.actualIsActive && treeItem.visibility != 4/*primitives.common.Visibility.Invisible*/) {
	    treeLevel.activeTreeItems.push(treeItem.id);
	}
...
*/
primitives.common.mergeSort = function (arrays, getItemWeight) {
    var result = null,
    ...
    switch (arrays.length) {
        case 0:
            ...
            break;
        case 1:
            result = arrays[0].slice(0);
            break;
        default:
            firstArray = arrays[0];
            for (arrayIndex = 1; arrayIndex < arrays.length; arrayIndex += 1) {
...
primitives.famdiagram.Controller.prototype._createOrgTree = function () {
    ...
    if (this.options.items.length > 0) {
        ...
        this._orgItemRoot = this._createOrgItem(this.maximumFamItemId, null /*parent id*/, null, this.minimumLevel - 1, null /* userItem */);
        this._orgItemRoot.hideParentConnection = true;
        this._orgItemRoot.hideChildrenConnection = true;
        this._orgItemRoot.title = "internal root";
        this._orgItemRoot.isVisible = false;
        ...
        for (index = 0, len = sortedFamilies.length; index < len; index+=1) {
            ...
            bestRootItem = this._orgItemRoot;
            bestReference = 0;
        ...
        /* scan _orgItemRoot, _orgItems hierarchy from root to bottom and balance _orgItemChildren collections */
        this._balanceChildrenForItem(extraGravities, grandChildren, this._orgItemRoot);
    }
};
...
primitives.famdiagram.Controller.prototype._showCursorNeighbours = function () {
    ...
    if (this._cursorTreeItem !== null) {
        ...
        if (this._cursorTreeItem.itemConfig != null) {
            ...
            itemsToCheck = famItem.children.slice(0);
            while (itemsToCheck.length > 0) {
                items = items.concat(itemsToCheck);
                newItemsToCheck = [];
                for (index = 0; index < itemsToCheck.length; index += 1) {
                    itemToCheck = this._famItems[itemsToCheck[index]];
                    orgItem = this._orgItems[itemsToCheck[index]];
                    if (!orgItem.isActive) {
                        newItemsToCheck = newItemsToCheck.concat(itemToCheck.children);
                    }
                    items = items.concat(itemToCheck.parents);
                }
                itemsToCheck = newItemsToCheck;
            }

            itemsToCheck = famItem.parents.slice(0);
            while (itemsToCheck.length > 0) {
                items = items.concat(itemsToCheck);
                newItemsToCheck = [];
                for (index = 0; index < itemsToCheck.length; index += 1) {
                    ...
                    orgItem = this._orgItems[itemsToCheck[index]];
                    if (!orgItem.isActive) {
                        newItemsToCheck = newItemsToCheck.concat(itemToCheck.parents);
                    }
                    items = items.concat(itemToCheck.children);
                }
                itemsToCheck = newItemsToCheck;
            }
...
primitives.orgdiagram.Controller.prototype._createOrgTree = function () {
	...
	if (this.options.items != null && this.options.items.length > 0) {
	    ...
	    for (index = 0, len = this.options.items.length; index < len; index += 1) {
	        ...
	        if (userItem.id != null) {
                ...
                */
	            ...
	            if (orgItem.parent == null) {
	                rootChildren.push(orgItem);

	                /* root item must be regular */
	                orgItem.itemType = 0/*primitives.orgdiagram.ItemType.Regular*/;
	            }
	    ...
	    this._orgItemRoot.id = maximumId;
	    this._orgItemRoot.hideParentConnection = true;
	    this._orgItemRoot.hideChildrenConnection = true;
	    this._orgItemRoot.title = "internal root";
	    this._orgItemRoot.isVisible = false;
	    ...
	    this._orgItemChildren[this._orgItemRoot.id] = rootChildren;
...
primitives.orgdiagram.Controller.prototype._hideRootConnectors = function (itemsToCheck) {
    var newItemsToCheck,
        index, len, index2, len2,
        itemToCheck, children, childItem, allRegular;
    while (itemsToCheck.length > 0) {
        newItemsToCheck = [];

        for (index = 0, len = itemsToCheck.length; index < len; index += 1) {
            itemToCheck = itemsToCheck[index];

            if (!itemToCheck.isVisible) {
                if (this._orgItemChildren.hasOwnProperty(itemToCheck.id)) {
                    children = this._orgItemChildren[itemToCheck.id];
                    allRegular = true;
                    for (index2 = 0, len2 = children.length; index2 < len2; index2 += 1) {
                        childItem = children[index2];
                    ...
                    if (allRegular) {
                        itemToCheck.hideChildrenConnection = true;

                        for (index2 = 0, len2 = children.length; index2 < len2; index2 += 1) {
                            childItem = children[index2];

                            childItem.hideParentConnection = true;
                            if (!childItem.isVisible) {
                                newItemsToCheck.push(childItem);
                            }
                        }
                    }
                }
...
primitives.orgdiagram.Controller.prototype._defineLogicalParent = function (logicalParentItem, treeItem) {
    ...
    */
    if (!logicalParentItem.actualIsActive || logicalParentItem.visibility == 4/*primitives.common.Visibility.Invisible*/) {
...
primitives.orgdiagram.TreeLevel.prototype.shiftDown = function (shift) {
    this.topConnectorShift += shift;
    this.connectorShift += shift;
    this.nextLevelShift += shift;
    this.shift += shift;    
};
